...
<script>
    ...
    document.getElementById('createPaymentForm').addEventListener('submit', async (e) => {
        ...
        try {
            ...
            showResult('createResult', '✅ Payment link created successfully', result, 'success', curlCommand);
    ...
    async function getPaymentDetails() {
        ...
        try {
            ...
            showResult('detailsResult', '✅ Payment details retrieved successfully', result, 'success', curlCommand);
        } catch (error) {
    ...
    async function listPayments() {
        ...
        try {
            ...
            if (result.results && result.results.length > 0) {
                ...
                if (result.next || result.previous) {
                    ...
                    paginationDiv.innerHTML = `
                            <div class="flex justify-between items-center">
                                ...
                                <div class="space-x-2">
                                    ${result.previous ? '<span class="text-blue-600">← Previous page available</span>' : ''}
                                    ${result.next ? '<span class="text-blue-600">Next page available →</span>' : ''}
                                </div>
                            </div>
                        `;
                ...
                noResults.innerHTML = '📭 No payments found for the current search criteria';
    ...
    function createPaymentCard(payment) {
        ...
        card.innerHTML = `
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-y-2 gap-x-4 text-sm text-gray-600">
                    <div><span class="font-medium">💰 Amount:</span> ${amount} SAR</div>
                    <div><span class="font-medium">📅 Created:</span> ${date}</div>
                    ${payment.single_use !== undefined ? `<div><span class="font-medium">🔄 Usage:</span> ${payment.single_use ? 'Single Use' : 'Multi Use'}</div>` : ''}
                    ${payment.expiry_date ? `<div><span class="font-medium">⏰ Expires:</span> ${new Date(payment.expiry_date).toLocaleDateString()}</div>` : ''}
                    ...
                    <div class="md:col-span-2"><span class="font-medium">👤 Creator:</span> ${payment.created_by_email || 'N/A'}</div>
                    ...
                    ${payment.client_first_name || payment.client_last_name ? `<div><span class="font-medium">👤 Customer:</span> ${[payment.client_first_name, payment.client_last_name].filter(Boolean).join(' ')}</div>` : ''}
                    ${payment.client_email ? `<div><span class="font-medium">📧 Email:</span> ${payment.client_email}</div>` : ''}
                    ${payment.client_phone_number ? `<div><span class="font-medium">📱 Phone:</span> ${payment.client_phone_number}</div>` : ''}
                </div>
    ...
    async function resolvePaymentLink() {
        ...
        if (!paymentId) {
            showResult('resolveResult', '❌ Error', { error: 'Please enter a payment link ID' }, 'error');
            return;
        }
        ...
        try {
            ...
            showResult('resolveResult', '✅ Payment link resolved successfully', result, 'success', curlCommand);
            ...
            const curlCommand = generateCurlCommand(`/payment_links/${paymentId}/resolve`, 'POST', {});
            let errorMessage = '❌ Error resolving payment link';
            let interpretation = '';

            if (error.message.includes('Unauthorized Origin')) {
                errorMessage = '❌ Unauthorized Origin Error';
                interpretation = `
    ...
    async function checkPaymentStatus() {
        ...
        try {
            ...
            showResult('statusResult', `⚡ Payment Status: ${status}`, statusSummary, 'success', curlCommand);
                    ...
                    <div class="mt-3 pt-2 border-t border-blue-200">
                        <div class="flex items-center justify-between">
                            <p class="text-xs text-blue-600">
                                💡 Payment ID has been auto-filled in other tabs
                            </p>
